...
<template>
  <form class="mb-1 flex max-w-96 flex-row gap-2">
    ...
    <label class="min-w-32 flex-grow">
      ...
      <input v-model.trim="sfname" class="bg-neutral-100 min-w-56" />
    </label>
    <label class="min-w-20 max-w-20">
      <span v-show="showHead" class="bg-muted block">TW-shade</span>
      <input v-model.number="shade" max="10" min="0" step="1" type="number" class="bg-neutral-100 min-w-20 max-w-20" />
    </label>
  </form>
</template>
