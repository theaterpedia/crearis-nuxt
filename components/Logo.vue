...
<template>
  <div :class="props.padding ? '' : 'pl-2 md:pl-4 lg:pl-6 xl:pl-8 ' + 'logoClass font-semibold dark:bg-black'">
    ...
    <h1 :class="props.logoSize === 'default' ? 'font-mona text-xl sm:text-2xl lg:text-3xl xl:text-4xl 2xl:text-5xl' : props.logoSize === 'sm' ? 'font-mona text-md sm:text-lg md:text-xl lg:text-2xl xl:text-3xl' : 'font-mona text-3xl sm:text-4xl lg:text-5xl 2xl:text-6xl'">
      <span :class="props.filled === true && props.extended === false ? 'text-white dark:text-black': 'text-primary-700'">Theater</span><span :class="filled && !extended ? 'text-secondary-400': 'text-secondary-700'">pedia</span>
    </h1>
  </div>
</template>
