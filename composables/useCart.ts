...
export const useCart = () => {
  ...
  const cartAdd = async (id: number, quantity: number) => {
    ...
    const { data, error } = await $sdk().odoo.mutation<MutationCartAddMultipleItemsArgs, CartAddItemResponse>(
  ...
  const updateItemQuantity = async (id: number, quantity: number) => {
    ...
    const { data, error } = await $sdk().odoo.mutation<MutationCartUpdateMultipleItemsArgs, CartUpdateItemResponse>(
  ...
  const removeItemFromCart = async (lineId: number) => {
    ...
    loading.value = true
    const { data, error } = await $sdk().odoo.mutation<MutationCartRemoveMultipleItemsArgs, CartRemoveItemResponse>(
