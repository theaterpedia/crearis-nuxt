<template>
  <Component 
    class="page-wrapper text-sm sm:text-base"
    :class="`page-bg-${backgroundColor}`"
  ...
  >
    ...
    <div v-if="alertBanner" class="alert-wrapper" :class="{ 'fullwidth-padded': fullwidthMode && fullwidthPadding }">
      <UiAlertBanner :alertType="alertBanner.alertType">
...
import type { TopnavParentItem, ToggleOption, ArrayOption } from '@crearis/ui'
import { 
  pageSettings, 
  layoutSettings, 
  navbarSettings, 
...
// PRUVIOUS: Define layout configuration
defineLayout({
  label: 'fulldefault',
  allowedBlocks: [
    'HeaderCtaImage',
