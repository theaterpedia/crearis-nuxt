<template>
  <Component :is="isSideNav ? 'Box' : 'div'" class="text-sm sm:text-base">
    ...
    <Main class="tl:px-8 ph:px-0 mx-auto max-w-screen-2xl px-12">
      <slot name="header">
        <Hero v-if="showHero" :imgTmp="imgTmp" contentAlignY="top" contentType="banner" contentWidth="full">
          <Component card="false" is="div" transparent>
            ...
            <template v-else>
              ...
              <div v-if="author" class="text-muted-text mt-2 text-xs">
                <span>Autor:</span>
                <span>{{ author }}</span>
              </div>
